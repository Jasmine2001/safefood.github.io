...
<div>
  <form 
    ...
    method="post"   
    action="/safefood/models/article_check.php"
  >
    <label>
    ...
    <label class="text-center mb-5 image-preview-wrapper" for="file-uploader">
      <img 
        id="img" 
        src="<?php echo (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http")."://". $_SERVER['HTTP_HOST']. '/safefood/src/img/up-arrow.png';?>" 
    ...
    success: function(data) {
      if(data.includes('文章新增成功')) {
        Swal.fire({
          ...
          if (result.value) {
            window.location = '/safefood/views/blog.php'
          }
        })
      }
    }
